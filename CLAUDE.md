...
(Cloud.ru) | **Код:** GitVerse `git@gitverse.ru:vacantrix/vacantrix-tg-bot.git` (основной) +
деплой-зеркало GitHub `github.com/vacantrix/vacantrix-tg-bot` (ПУБЛИЧНОЕ — источник для Render;
Render не умеет GitVerse, история проверена на секреты)
...
1. `git push gitverse main && git push origin main` (gitverse = основной хостинг кода,
   origin/GitHub = ПУБЛИЧНОЕ деплой-зеркало для Render; секретов в коде/истории нет — проверено).
2. Дашборд Render → сервис → **Manual Deploy** (сервис создан по Public Git Repository URL
   `https://github.com/vacantrix/vacantrix-tg-bot` — авто-деплоя у этого режима нет).
